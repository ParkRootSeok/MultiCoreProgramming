...
int main() {
	...
	for (int i = 0; i < 2; i++) {
		queue[i] = clCreateCommandQueueWithProperties(context, device, CL_QUEUE_PROFILING_ENABLE, &err);
		CHECK_ERROR(err);
	}
	...
	const char* kernel_source_code = get_source_code("int_overlap_kernel.cl", &kernel_source_size);
