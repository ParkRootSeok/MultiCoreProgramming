...
int main() {
	...
	const char* kernel_source_code = get_source_code("red_kernel.cl", &kernel_source_size);
	...
	CHECK_ERROR(err);
	err = clEnqueueWriteBuffer(queue, sumBuffer, CL_TRUE, 0, sumSize, sum, 0, NULL, NULL);
	CHECK_ERROR(err);
	...
	printf("Not parallel Result : %d(sum = %d)\n", result / capacity, result);
	...
	CHECK_ERROR(err);
	err = clSetKernelArg(kernel, 2, sizeof(cl_int) * 256, NULL);
	CHECK_ERROR(err);
	err = clSetKernelArg(kernel, 3, sizeof(cl_int), &capacity);
	CHECK_ERROR(err);
